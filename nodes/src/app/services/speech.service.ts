...
export class SpeechService {
  ...
  transcribeAudio(audioFile: Blob) {
    ...
    if (!['audio/mp3', 'audio/mp4', 'audio/aac', 'audio/wav', 'audio/flac', 'audio/ogg', 'audio/opus', 'audio/wma', 'audio/m4a'].includes(mimeType)) {
      console.error("Ungültiges Dateiformat. Die Datei muss eines der folgenden Formate haben: mp3, mp4, aac, wav, flac, ogg, opus, wma, m4a.");
      return;
    }
  ...
  public async convertToMp3(blob: Blob): Promise<Blob> {
    ...
    // Write file
    await ffmpeg.writeFile('recording.webm', new Uint8Array(await blob.arrayBuffer()));
